...
It is currently a character type. This is because it came from the column headers of the original table, and R treats column names as text by default. It should be numeric so we can plot it on a continuous x-axis.
...
-   **Question:** What does an `NA` mean in this context? *Hint:* The data come from the university registrar, and they have records on every single graduates, there shouldn't be anything "unknown" to them about who graduated when.
 ...
 NA means that zero students graduated with that degree in that year. AKA represents a count of 0, not missing info
